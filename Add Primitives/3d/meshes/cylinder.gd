...
func build_mesh(params, smooth = false, reverse = false):
	...
	for i in range(c):
		...
		uv = [(Vector2(1.0, (float(i)/c)/2) + uv_offset),\
		             ...
		             (Vector2(float(s - 1)/s, (float(i+1)/c)/2) + uv_offset),\
		             (Vector2(float(s - 1)/s, (float(i)/c)/2) + uv_offset)]
		             
		add_quad([circle[0] + min_pos, circle[0] + next_cut,\
		          circle[s - 1] + next_cut, circle[s - 1] + min_pos], uv, reverse)
